 ...
 * thing — it exists to exercise the editor's data flow (the synthesised
 * palette/threshold/stats fields, source-dependent field visibility, the
 * write-only-non-defaults pruning), not its looks in HA. Multi-value
 * selectors in particular are plain comma-separated text boxes here, where
 * real HA gives you a chip picker.
 */
