 ...
 *
 * **Accents from `SchemeVibrant` seeded #ff5c73** (this card's original
 * accent). **Surfaces from `SchemeNeutral` seeded #4da3ff** — the shared
 * house neutral, byte-identical to the block in `activity-heatmap` and
 * `quick-toggles` (and within a hair of `garage-control`), so all four cards
 * agree on a dashboard. Surface tint and accent vividness are independent
 * choices; this card pulls them apart hard.
 *
 * The surfaces went through two wrong answers before this one, both worth
 * recording. Seeding the neutrals from the card's own red gave a warm
 * pink-grey plate — the tint is only chroma ~2, but it is unmistakable
 * beside a cooler card. Replacing that with `SchemeMonochrome` produced
 * literally equal-channel greys and still read yellowish, because a
 * perfectly neutral grey looks warm next to the slightly cool grey the other
 * cards use. **Neutral in isolation is not the target; matching the
 * neighbours is.** The house block is a touch blue (#1b1b1d, blue channel
 * two steps up) and that is what actually reads as grey here.
 *
 * Grey rather than tinted also matters on its own terms: the card shows
 * three semantic colours side by side — on-target (success), slipping
 * (warning) and off-target (error) — and any tint biases all three against
 * each other.
 *
 ...
 * M3 ships.** The spec defines `error` and nothing else semantic, but this
 * card's adherence ring is a genuine three-step scale, not a decorative
 * accent. They're built the way M3 builds custom colours: a tonal palette
 * from one seed (#0f9f78 and #c98513, this card's original good/warn
 * colours), sampled at the standard custom-colour tones — light takes
 * T40/T100/T90/T10 and dark T80/T20/T30/T90 for
 * colour/on-colour/container/on-container. Same contrast guarantees as the
 * built-in roles instead of a hand-mixed rgba.
 *
 * `error` genuinely carries "off-target" here rather than merely "red":
 * missing the visit goal is the condition this card exists to surface, and
 * money wasted is its cost. That's the M3 meaning of error, so it isn't
 * being borrowed for decoration.
 */
export const m3Tokens = css`
  :host {
    /* ---- colour: dark scheme, seed #ff5c73 ---- */
    /* accents: SchemeVibrant */
    --m3-primary: #ffb2b7;
    --m3-on-primary: #67001c;
    --m3-primary-container: #91002b;
    --m3-on-primary-container: #ffdadb;
    --m3-secondary: #f6b8aa;
    --m3-on-secondary: #4c261c;
    --m3-secondary-container: #663b31;
    --m3-on-secondary-container: #ffdad2;
    --m3-tertiary: #ffb68d;
    --m3-on-tertiary: #502405;
    --m3-tertiary-container: #6c3a19;
    --m3-on-tertiary-container: #ffdbc9;
    --m3-error: #ffb4ab;
    --m3-on-error: #690005;
    --m3-error-container: #93000a;
    --m3-on-error-container: #ffdad6;
    /* surfaces: SchemeNeutral seeded #4da3ff — the shared house neutral */
    --m3-surface: #131314;
  ...
  :host([data-theme="light"]) {
    ...
    --m3-primary-container: #ffdadb;
    --m3-on-primary-container: #40000e;
    --m3-secondary: #825247;
    --m3-on-secondary: #ffffff;
    --m3-secondary-container: #ffdad2;
    --m3-on-secondary-container: #33110a;
    --m3-tertiary: #88512e;
    --m3-on-tertiary: #ffffff;
    --m3-tertiary-container: #ffdbc9;
    --m3-on-tertiary-container: #321200;
    --m3-error: #ba1a1a;
    --m3-on-error: #ffffff;
