 ...
 *
 * **Accents from `SchemeVibrant` seeded #00cfff** (this card's original
 * cyan, and the same seed `pc-control` uses — the two agree exactly).
 * **Surfaces from `SchemeNeutral` seeded #4da3ff** — the shared house
 * neutral, byte-identical to `activity-heatmap`, `quick-toggles`,
 * `gym-tracker`, `pc-control` and `schedule-timeline-card`, so every card
 * sits on the same plate.
 *
 * Note the surfaces are deliberately NOT seeded from this card's own cyan.
 * `geyser-control` does tint its surfaces, because a single accent meaning
 * "hot" is that card's whole identity; here the accent means "water" and a
 * blue-tinted plate under a blue accent just reads as a blue card. Surface
 * tint and accent vividness are independent choices.
 *
