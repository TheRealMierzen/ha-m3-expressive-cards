 ...
 *
 * **Accents from `SchemeVibrant` seeded #00cfff** (this card's original cyan).
 * **Surfaces from `SchemeNeutral` seeded #4da3ff** — the shared house neutral,
 * byte-identical to the block in `activity-heatmap`, `quick-toggles` and
 * `gym-tracker`, so the cards agree on a dashboard. Seeding the neutrals from
 * a card's own accent tints the plate toward that hue, which is wrong here for
 * two reasons: this card is the densest in the set (eight metric tiles, two
 * automation rows, three progress bars and six status badges on screen at
 * once), so its plate has to stay out of the way; and it carries four
 * semantic colours simultaneously — good, warning, bad and info — which any
 * tint biases against each other.
 *
 ...
 * card's status dot, temperature tile and SMART/firmware badges are all
 * three- and four-step health scales. They're built the way M3 builds custom
 * colours: a tonal palette from one seed (#0f9f78 and #c98513, the same two
 * the other migrated cards use) sampled at the standard custom-colour tones —
 * light takes T40/T100/T90/T10 and dark T80/T20/T30/T90 for
 * colour/on-colour/container/on-container.
 *
 * "Info" from the old scheme is not a custom colour: it was cyan, which is
 * this card's `primary`, so it maps onto the accent it always was.
 *
