 ...
 *
 * **Accents from `SchemeVibrant` seeded #00cfff** — the water cyan
 * `irrigation-control` and `pc-control` already use, and byte-identical to
 * both. Two irrigation cards on one dashboard should agree about what water
 * looks like.
 ...
 *
 * ## The extended "dry" role
 *
 * This card has a second state colour that is neither an accent nor an
 * error: a bucket below field capacity. Nothing is wrong when a zone is dry
 * — that is the integration working — so `error` would overstate it, and the
 * water accent would say the opposite of what it means. It is generated the
 * way M3-EXPRESSIVE.md's "Extended semantic colours" section says, as a
 * `TonalPalette` off seed #e08b3a sampled at the standard custom-colour
 * tones (dark: T80/T20/T30/T90, light: T40/T100/T90/T10), so it carries the
 * same contrast guarantees as the built-in roles.
 *
 * **`--m3-dry` and `--m3-primary` are the same luminance — 1.00:1, in both
 * themes.** They are M3 tones T80/T40 off different hues, so pitfall 16
 * applies: hue is the only channel separating them. That is fine *here* and
 * it is deliberate, because hue is not the channel doing the work. A
 * deficit fill hangs *below* the gauge's zero line and a surplus fill sits
 * *above* it — they are mutually exclusive within a zone and never adjacent
 * — the sump is hatched where the water column is solid, and the verdict
 * beside the gauge says which in words. Don't "fix" this by re-toning one of
 * them; do keep all four channels if you rework the gauge.
 *
 * `--m3-dry` is also 1.00:1 against `--m3-error` for the same reason. They
 * are never the same kind of object: `error` only ever paints the card-level
 * problem banner (full width, alert icon, the integration's own `reason`
 * text), `dry` only ever paints things inside a zone tile.
 */
export const m3Tokens = css`
  :host {
    /* ---- colour: dark scheme ---- */
    /* accents: SchemeVibrant #00cfff */
    --m3-primary: #54d5ff;
    --m3-on-primary: #003544;
    --m3-primary-container: #004d61;
    --m3-on-primary-container: #b8eaff;
    --m3-secondary: #accae5;
    --m3-on-secondary: #133348;
    --m3-secondary-container: #2c4a60;
    --m3-on-secondary-container: #cae6ff;
    --m3-tertiary: #abc8f7;
    --m3-on-tertiary: #0f3158;
    --m3-tertiary-container: #2a4870;
    --m3-on-tertiary-container: #d4e3ff;
    --m3-error: #ffb4ab;
    --m3-on-error: #690005;
    --m3-error-container: #93000a;
    --m3-on-error-container: #ffdad6;
    /* extended semantic role: soil-moisture deficit. TonalPalette #e08b3a */
    --m3-dry: #ffb77a;
    --m3-on-dry: #4c2700;
    --m3-dry-container: #6d3a00;
    --m3-on-dry-container: #ffdcc2;
  ...
  :host([data-theme="light"]) {
    ...
    --m3-primary-container: #b8eaff;
    --m3-on-primary-container: #001f28;
    --m3-secondary: #446279;
    --m3-on-secondary: #ffffff;
    --m3-secondary-container: #cae6ff;
    --m3-on-secondary-container: #001e30;
    --m3-tertiary: #435f89;
    --m3-on-tertiary: #ffffff;
    --m3-tertiary-container: #d4e3ff;
    --m3-on-tertiary-container: #001c3a;
    --m3-error: #ba1a1a;
    --m3-on-error: #ffffff;
    ...
    --m3-dry: #8f4e00;
    --m3-on-dry: #ffffff;
    --m3-dry-container: #ffdcc2;
    --m3-on-dry-container: #2e1500;
    --m3-surface: #fbf9fa;
